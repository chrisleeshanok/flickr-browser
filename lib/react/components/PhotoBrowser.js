...
var PhotoBrowser = React.createClass({
    ...
    render: function () {
        ...
        var photoComponents1 = photosCopy.splice(0, Math.floor(this.state.photos.length/2)).map(function(photo) {
            return <Photo countModifier={this.props.countModifier} removeHandler={this.removePhoto} ownerId={photo.owner} title={photo.title} ownername={photo.ownername} key={photo.id} farmId={photo.farm} serverId={photo.server} photoId={photo.id} secret={photo.secret}/>
        }.bind(this));
